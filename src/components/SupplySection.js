...
export default function SupplySection({
  ...
  const totalSupplied = tokens.reduce((sum, token) => {
    ...
    return (
      <Box p={4} borderWidth="1px" borderRadius="md" bg="#1e1e1e">
        <HStack justify="space-between" mb={4}>
          <Text fontWeight="semibold" fontSize="lg">Supplies</Text>
          <Button 
            onClick={onAddClick}
            colorScheme="blue"
            size="md"
  ...
  return (
      ...
      <Box mb={4}>
        <Text fontSize="sm" color="gray.500">Total Supplied</Text>
        <Text fontSize="xl" fontWeight="bold">${formatPrice(totalSupplied)}</Text>
      </Box>
